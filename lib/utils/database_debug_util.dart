    ...
    try {
      ...
      if (transactionResult.isNotEmpty) {
        for (var i = 0; i < transactionResult.length; i++) {
          ...
          print(
            '${i + 1}. ID: ${txn['transaction_id']}, Type: ${txn['transaction_type']}, Amount: ${txn['amount']}, Status: ${txn['status']}, Date: ${txn['transaction_date']}',
          );
        }
      } else {
    ...
    try {
      ...
      if (result.isNotEmpty) {
        for (var account in result) {
          print(
            'ID: ${account['account_id']}, Username: ${account['username']}, Balance: ${account['balance']}',
          );
        }
      } else {
